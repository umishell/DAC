...
- [x] `GET /health` público → `{ "status": "UP" }` **sem** `_links`
- [x] ACL: `/health` na lista pública ([`acl.ts`](backend/gateway/src/auth/acl.ts))
- [x] Dockerfile + healthcheck do serviço `gateway` (porta **3000** no host)
